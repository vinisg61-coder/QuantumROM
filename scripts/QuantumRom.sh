...
APPLY_STOCK_ROM_FLOATING_FEATURE() {
    ...
    # camera fix: S23FE camera HAL needs image_codec.samsung.v2 declared even
    # when the target stock floating feature lacks the key entirely.
    _VLIB_FIX="$(GET_FF_VALUE "SEC_FLOATING_FEATURE_CAMERA_CONFIG_VENDOR_LIB_INFO" "$STOCK_ROM_FLOATING_FEATURE")"
    if [ -z "$_VLIB_FIX" ]; then
        _VLIB_FIX="image_codec.samsung.v2"
    elif [[ "$_VLIB_FIX" != *"image_codec.samsung.v2"* ]]; then
        _VLIB_FIX="${_VLIB_FIX},image_codec.samsung.v2"
    fi
    UPDATE_FLOATING_FEATURE "$FLOATING_FEATURE_FILE_DIRECTORY" \
